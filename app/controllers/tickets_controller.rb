class TicketsController < ApplicationController
  ...
  def index
    ...
    @tickets = Ticket.last(200).to_a.keep_if{|ticket| ticket.ticket_time && ticket.completed? || ticket.completable? || ticket.confirmable? || ticket.confirmed? || ticket.finalizing? || ticket.booked? || ticket.payment_complete? || ticket.ready_to_book? || ticket.waiting_for_review?}
    @tickets = @tickets.sort! { |a,b| b.ticket_time.date <=> a.ticket_time.date }
  ...
  private
    ...
    def ticket_params
      params.require(:ticket).permit(:requester_id, :deposit_status, :lesson_time_id, :activity, :requested_location, :requester_name, :phone_number, :state, :actual_start_time, :terms_accepted, :guest_email, :how_did_you_hear, :num_days, :booking_order_value, :is_gift_voucher, :gift_recipient_email, :gift_recipient_name, :product_id, :admin_price_adjustment, :promo_code_id, :planned_start_time, :payment_status, :payment_method, :payment_date, :additional_info, :ticket_type, :street_address, :city, :state_code, :zip_code, :drivers_license, :skip_validations, :administrator_notes, :multi_product_order, :refund_issued,
      lesson_time_attributes: [:date, :slot],
      students_attributes: [:id, :name, :age_range, :gender, :_destroy, :requester_id], 
      )
    end
